...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    //遥控器设置模式//从遥控器接受到模式的选择
		chassis_behaviour_mode = CHASSIS_NO_FOLLOW_YAW;
		
    if (switch_is_mid(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))//如果rc_ctrl的s【0】值为3
    {
        ...
        chassis_behaviour_mode = CHASSIS_NO_FOLLOW_YAW;//地盘不跟随角度
    }
    else if (switch_is_down(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))//如果rc_ctrl的s【0】值为2
    {
        chassis_behaviour_mode = CHASSIS_NO_MOVE;//地盘不动
    }
    else if (switch_is_up(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))//如果rc_ctrl的s【0】值为1
    {
      //  chassis_behaviour_mode = CHASSIS_ENGINEER_FOLLOW_CHASSIS_YAW;//底盘跟随云台
    }
...
//    }
    ...
    //根据行为模式选择一个底盘控制模式//根据遥控器收到的选择对地盘进行控制
...
static void chassis_engineer_follow_chassis_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    *angle_set = rad_format(chassis_move_rc_to_vector->chassis_yaw_set - CHASSIS_ANGLE_Z_RC_SEN * chassis_move_rc_to_vector->chassis_RC->rc.ch[CHASSIS_WZ_CHANNEL]);//遥控器通道2（左摇杆）
}

...
#define USART_SendData HAL_UART_Transmit
#define USART_GetFlagStatus __HAL_UART_GET_FLAG

int fputcc(uint8_t ch)
{
		/* 发送一个字节数据到USART1 */
		USART_SendData(DEBUG_USART, &ch, 1, 0xffff);
		
		/* 等待发送完毕 */
		while (USART_GetFlagStatus(DEBUG_USART, UART_FLAG_TXE) == RESET);		
